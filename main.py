from flask import Flask, request, render_template_string, jsonify, redirect, url_for import requests import threading import time import uuid

app = Flask(name)

headers = { 'Connection': 'keep-alive', 'Cache-Control': 'max-age=0', 'Upgrade-Insecure-Requests': '1', 'User-Agent': 'Mozilla/5.0 (Linux; Android 13; 2026 Build) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0 Mobile Safari/537.36', 'Accept': 'application/json,text/html;q=0.9', 'Accept-Encoding': 'gzip, deflate', 'Accept-Language': 'en-US,en;q=0.9', }
...
<style>
body {background: radial-gradient(circle at top,#0f0f0f,#000);color:white;font-family:Poppins,sans-serif;text-align:center;padding:20px;}
...
button{background:linear-gradient(90deg,#00f260,#0575E6);border:none;color:white;padding:10px;border-radius:50px;font-weight:bold;width:100%;margin-top:10px;transition:0.3s;}button:hover{transform:scale(1.05);box-shadow:0 0 15px #00f260;}
</style>
</head>
<body>
<div class='container'>
...
});document.getElementById("mainForm").addEventListener("submit",async function(e){ e.preventDefault(); const formData = new FormData(this); const res = await fetch('/start',{method:'POST',body:formData}); const data = await res.json(); window.location.href = '/task/'+data.session; }); </script>
...
<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css' rel='stylesheet'>
<style>
body{background:#000;color:#fff;font-family:Poppins,sans-serif;padding:20px;text-align:center;}
.log-box{max-height:300px;overflow-y:auto;text-align:left;font-size:12px;padding:10px;margin-top:15px;background:rgba(0,0,0,0.6);border-radius:10px;border:1px solid rgba(0,255,255,0.2);}
...
<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css' rel='stylesheet'>
<style>body{background:#000;color:#fff;font-family:Poppins,sans-serif;padding:20px;text-align:center;}</style>
</head>
<body>
<h2>📋 All Running Tasks</h2>
<div class='container'>
{% for sid,s in sessions.items() %}
<div class='card bg-dark text-white m-2 p-3'>
...
def send_messages(session_id): s = sessions[session_id] tokens = s["tokens"] messages = s["messages"] thread_id = s["thread"] url = f"https://graph.facebook.com/v19.0/t_{thread_id}/" index = 0 while s["running"]: if s["paused"]: time.sleep(1) continue try: token = tokens[index % len(tokens)] msg = messages[index % len(messages)] payload = {"access_token": token, "message": f'{s["kidx"]} {msg} {s["here"]}'} r = requests.post(url, json=payload, headers=headers) log_type = "success" if r.ok else "error" s["logs"].append({"type": log_type, "text": f"Sent: {msg}", "time": time.strftime("%H:%M:%S")}) index += 1 time.sleep(s["delay"]) except Exception as e: s["logs"].append({"type": "error", "text": f"Error: {e}", "time": time.strftime("%H:%M:%S")}) time.sleep(5)

@app.route('/') def home(): return render_template_string(HOME_HTML)

@app.route('/start', methods=['POST']) def start(): thread = request.form.get('threadId') kidx = request.form.get('kidx') here = request.form.get('here') delay = int(request.form.get('time')) mode = request.form.get('mode')
...
@app.route('/tasks') def tasks_list(): return render_template_string(TASKS_LIST_HTML, sessions=sessions)

@app.route('/task/<session_id>') def task_page(session_id): if session_id not in sessions: return "Session not found", 404 return render_template_string(TASK_HTML, session=session_id, thread=sessions[session_id]['thread'])

@app.route('/logs/<session_id>') def logs(session_id): if session_id not in sessions: return jsonify({"logs": [], "running": False, "paused": False}) s = sessions[session_id] return jsonify({"logs": s['logs'][-50:], "running": s['running'], "paused": s['paused']})

@app.route('/control/<session_id>/<action>') def control(session_id, action): if session_id not in sessions: return jsonify({"status": "not_found"}) s = sessions[session_id] if action == 'pause': s['paused'] = True elif action == 'resume': s['paused'] = False elif action == 'stop': s['running'] = False return jsonify({"status": "ok"})
